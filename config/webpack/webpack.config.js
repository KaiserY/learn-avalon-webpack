...
module.exports = (option = {
    ...
    ((debug) => {
        ...
        function getEntry(option, VIEWS) {
            ...
            let files = glob.sync(option.globPath);
            ...
            let baseName; // 文件名
            let pathName; // 文件夹路劲
            let relativeName; // 键名所需,相对传入文件地址路径
            let extName; // 文件格式
    ...
    (config, htmlFiles, htmlPath, viewType, debug) => {
        ...
        htmlFiles.forEach(function(pathname) {
            ...
            conf.chunksSortMode = function(a, b) { // 按照配置排序
                let index = {};
                let i = 1;
                let len = conf.chunks.length;
                for (; i <= len; i++) {
                    index[conf.chunks[len - i]] = i;
                }
                let aI = index[a.origins[0].name];
                let bI = index[b.origins[0].name];
